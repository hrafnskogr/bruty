...
def main(argv):
	...
	pass_lines = 1		# for stats, set to 1 to avoid dividing by 0 O_O

	# password will be either generated by a bruteforcer or picked from a file
	if(_args.brute):
		generator = brutor(_lo+_up+_nu+_sp, _args.c_min, _args.c_max)
		pass_lines = generator.get_keyspace_size()
		#_parser = Process(target=pooler_brute, args=(_args.tasks, _qPass, _lo+_up+_nu+_sp, _args.c_min, _args.c_max))
		_parser = Process(target=pooler_brute, args=(_args.tasks, _qPass, generator))
		_parser.start()
	else:
	...
	if(_args.ssh):
		for i in range(0, _args.tasks):
			_procs.append(Process(target=mods.requestor_ssh.worker, args=(_qPass, _qRes, _args.logins, _args.url, count, ended, i)))
	elif(_args.gpg):
		for i in range(0, _args.tasks):
			_procs.append(Process(target=mods.requestor_gpg.worker, args=(_qPass, _qRes, _args.url, count, ended, i)))
	else:
		http_session = requests.Session() # requests optimisation, less tcp handshakes...
		for i in range(0, _args.tasks):
			_procs.append(Process(target=mods.requestor_http.worker, args=(_qPass, _qRes, _args.logins, _args.url, http_session , count, ended, _args.head, _args.timeout, i)))
...
def pooler_brute(nProcs, qPass, generator):
	pool_size = 200 * nProcs
...
def usage():
	...
	\t--end
	\t-H\t	Use HTTP HEAD request when attacking HTTP BasicAuth. Default attack: HTTP GET
	\t--head
	\t-S\t	Attack ssh
	\t--ssh 
	\t-G\t	Attack GPG encoded file
	\t--gpg\t
...
def args_inquisitor():
	...
	mod_mutex.add_argument('-G', '--gpg', help="Brute-force GPG", action="store_true", default=False)
